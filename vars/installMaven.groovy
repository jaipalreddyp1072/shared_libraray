def call(String targetHost, String sshKeyId = 'ec2-ssh-key', String mavenVersion = '3.9.6') {
    // Install Maven on a remote server via SSH
    withCredentials([sshUserPrivateKey(credentialsId: sshKeyId, keyFileVariable: 'SSH_KEY')]) {
        sh """
            ssh -i $SSH_KEY -o StrictHostKeyChecking=no ${targetHost} << 'EOF'
                set -e
